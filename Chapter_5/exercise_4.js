...
function characterCount(script) {
  return script.ranges.reduce(function (current, [from, to]) {
...
function characterScript(code) {
  for (let script of SCRIPTS) {
    if (
      script.ranges.some(function ([from, to]) {
...
let lsitOfNumbers = countBy(
  [1, 2, 3, 14, 15, 14, 13, 4, 5, 13, 4, 14, 16, 14, 15, 12],
  function (number) {
    if (number % 2 === 0) {
      return "even";
    } else {
...
function textSrcipt(text) {
  ...
  let total = scripts.reduce(function (n, { count }) {
    return n + count;
  }, 0);

  if (total === 0) return "no text fond";
...
function dominantDirection(text) {
  ...
  return scripts.reduce(function (a, b) {
    if (a.count > b.count) {
      return a;
    } else {
      return b;
